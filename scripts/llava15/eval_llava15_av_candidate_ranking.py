#!/usr/bin/env python3
"""Evaluate AV candidate ranking for LLaVA-NLA.

For each injected activation, score a pool of candidate explanations by
teacher-forced NLL:

  activation -> replace <image> marker embedding -> language model
...
def main() -> None:
    ...
    parser.add_argument("--injection-scale", type=float, default=31.46)
    ...
    prompt_ids, inj_positions = encode_prompt(
        ...
        int(token_meta["injection_left_neighbor_id"]),
        int(token_meta["injection_right_neighbor_id"]),
        str(token_meta.get("injection_token", "<image>")),
